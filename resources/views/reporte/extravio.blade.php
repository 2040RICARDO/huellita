  ...
  <div id="main-container">
    <div class="flex-container">
      ...
      <table align="center"  width="100%">
        <thead>
          <tr align="center">
              <td colspan="" style="font-size: 120%; font-weight: bold;">EXTRAVIOS</strong></td>
          </tr>
        </thead>
      </table>   
      <br>
      ...
      <table align="center"  border="1" >
        <thead>
          ...
          <tr style="background-color: #CACFD2; color: black;font-weight: bold; font-size: 15px;" class='content' id="cabeza">
            ...
            <th style="font-size: 85%;" align="center">DUEÑO</th>
            ...
            <th style="font-size: 85%;" align="center">FECHA EXTRAVIO</th>
        ...
        <tbody>
            @php($count=1)
            @foreach ($extravios as  $value)
                <tr class='content'>
                    <td style="font-size: 80%;">{{$count++}}</td>
                    <td style="font-size: 80%;">{{$value->nombre}} {{$value->apellidos}}</td>
                    <td style="font-size: 80%;">{{$value->ci}}</td>
                    <td style="font-size: 80%;">{{$value->direccion}}</td>
                    <td style="font-size: 80%;" align="center">{{$value->celular}}</td>
                    <td style="font-size: 80%;" align="center">{{$value->fecha_extravio}}</td>
                    <td style="font-size: 80%;width: 100px" align="center">{{$value->descripcion}}</td>
                    <td style="font-size: 80%;" align="center">{{$value->estado == true?'EN BUSQUEDA':'ENCONTRADO'}}</td>
                </tr>
            @endforeach
        </tbody>
      </table>
