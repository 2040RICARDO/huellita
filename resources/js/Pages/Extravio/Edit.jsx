...
export default function EditExtravio({auth,extravio}){
    const { data, setData, post, processing, errors } = useForm({
        nombre: extravio.nombre,
        apellidos: extravio.apellidos,
        ci: extravio.ci,
        direccion: extravio.direccion,
        celular: extravio.celular,
        fechaExtravio: extravio.fecha_extravio,
        descripcion: extravio.descripcion,
        fotografia: extravio.imagen_extravio!= null?extravio.imagen_extravio:null,
        estado:extravio.estado,
    });

    const submit = (e) => {
        e.preventDefault();
        post(route('extravio_a.actualizar', { id: extravio.id }));
    };
    ...
    useEffect(() => {
        if(extravio.imagen_extravio != null){
            setPreviewImage(`/img_extravio/${extravio.imagen_extravio}`)
        }
  ...
  return (
    ...
    <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Editar Extravio</h2>}
        >
