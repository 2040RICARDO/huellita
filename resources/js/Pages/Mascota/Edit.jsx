...
export default function EditMascota({auth,refugios,animales,mascota}){
    const { data, setData, post, processing, errors } = useForm({
        ...
        estado_vida:mascota.estado_vida,
        fotografia: mascota.imagen_mascota != null?mascota.imagen_mascota:null,
        tipoId:mascota.tipoanimal_id,
        refugioId: mascota.refujio_id,
    });
    ...
    useEffect(() => {
        if(mascota.imagen_mascota != null){
            setPreviewImage(`/img_mascota/${mascota.imagen_mascota}`)
        }
    }, [])
    
    const submit = (e) => {
        e.preventDefault();
        post(route('mascota_a.actualizar', { id: mascota.id }));
        //put(route('mascota.update', { id: mascota.id }));
    };
                            ...
                            <div className="flex items-center space-x-4">
                                <label className="inline-flex items-center rounded-lg border border-gray-300 p-2 hover:border-blue-500 transition-all duration-300 mr-16" >
                                    <input
                                        ...
                                        name="estado_vida"
                                        value="1"
                                        checked={data.estado_vida === 1}
                                        onChange={(e) => setData('estado_vida', 1)}
                                    />
                                    <span className="ml-2 text-sm font-semibold">VIVO</span>
                                </label>
                                <label className="inline-flex items-center rounded-lg border border-gray-300 p-2 hover:border-blue-500 transition-all duration-300">
                                    <input
                                        type="radio"
                                        ...
                                        name="estado_vida"
                                        value="0"
                                        checked={data.estado_vida === 0}
                                        onChange={(e) => setData('estado_vida', 0)}
                                    />
                                    <span className="ml-2 text-sm font-semibold">MUERTO</span>
                                </label>
                            </div>
                            <InputError className="text-red text-xs italic" message={errors.genero} />
                        </div>
