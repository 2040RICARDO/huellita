...
export default function IndexMascota ({auth,mascotas,animales}) {
    ...
    const handleToggleMenu = (mascotaId) => {
        setShowMenus((prevMenus) => ({
            ...prevMenus,
            [mascotaId]: !prevMenus[mascotaId],
        }));
    };
    ...
    const HandleDelete = (e,id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.delete(route('mascota.destroy',id));
            }
        });
    };
    ...
    const searchMascotas = () => {
        return mascotas.filter((mascota) => {
            return (
                mascota.nombre.toLowerCase().includes(searchQuery.toLowerCase())  ||
                mascota.tipo.toLowerCase().includes(searchQuery.toLowerCase())  ||
                mascota.genero.toLowerCase().includes(searchQuery.toLowerCase()) 
            );
        });
    };

    const filteredMascotas = searchMascotas();
     ////////////////busqueda

     ////////////paginacion
    const [pageNumber, setPageNumber] = useState(0);
    const mascotasPerPage = 30; 
    const pagesVisited = pageNumber * mascotasPerPage;
    ...
    const handleReporte = () => {
        const estado_vida = data.estado_vida;
        const adoptado = data.adoptado;
        const tipo_animal = data.tipo_animal;
        window.open( `/mascota_r/reporte/${estado_vida}/${adoptado}/${tipo_animal}`, '_blank');
    };
    /////REPORTE
  ...
  return (
    <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Mascotas</h2>}
        >
        ...
        <section className="container px-4 mx-auto">
            <div className="sm:flex sm:items-center sm:justify-between">
                <div>
                    <div className="flex items-center gap-x-3">
                        ...
                        <span className="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full  ">{mascotas.length} Mascotas</span>
                    </div>

                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-300">Se tiene registrado en Total {mascotas.length} Mascotas</p>
                </div>
                ...
                <div className="flex items-center mt-4 gap-x-3">
                    ...
                    <Link href={route('mascota.create')}  className="flex items-center justify-center w-1/2 px-5 py-2 text-sm tracking-wide text-white transition-colors duration-200 bg-blue-500 rounded-lg shrink-0 sm:w-auto gap-x-2 hover:bg-blue-600 dark:hover:bg-blue-500 dark:bg-blue-600">
                        <PlusCircle size={15}/>
                        ...
                        <div className="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg">
                            <table className="min-w-full divide-y divide-gray-200 ">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700">
                                  {
                                    filteredMascotas.slice(pagesVisited, pagesVisited + mascotasPerPage).map((mascota,index)=>(
                                        <tr>
                                            ...
                                            <td className="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                                <div>
                                                    <h2 className="font-medium text-gray-800 te ">{mascota.nombre}</h2>
                                                    <p className="text-xs font-normal text-gray-600 dark:text-gray-400"  >{mascota.tipo}</p>
                                                </div>
                                            </td>
                                            <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                <div className={`inline-flex items-center px-3 py-1 rounded-full gap-x-2 ${mascota.genero == 'MACHO'?'text-emerald-500 bg-blue-100/60':'text-black-500 bg-pink-100/60'}  `}>
                                                           
                                                    <h2 className="text-sm font-normal">{mascota.genero}</h2>
                                                </div>
                                     
                                            </td>
                                            <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                <h4 className="text-gray-700 ">{mascota.descripcion}</h4> 
                                            </td>
                                            <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                <h4 className="text-gray-700 ">{mascota.refugio}</h4> 
                                            </td>

                                            <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                <div>
                                                    <button
                                                        onClick={() => handleToggleMenu(mascota.id)}
            ...
            {selectedMascota && (
                    <div className="relative flex w-full max-w-[48rem] flex-row rounded-xl bg-white bg-clip-border text-gray-700 shadow-md">
                        ...
                        {selectedMascota.imagen_mascota != null?
                        (
                            <img
                                src={`/img_mascota/${selectedMascota.imagen_mascota}`}
                                alt="image"
                        ...
                        <div className="flex flex-row text-sm">
                                ...
                                <p className="flex items-center  text-gray-500 ml-3 text-center">
                                    ...
                                    <span >{selectedMascota.descripcion != null?selectedMascota.descripcion:''}</span>
                                </p>
                            </div>
                        </div>
                        <div className="flex flex-col space-y-4  p-6 text-gray-600">
                            ...
                            <CornerDownRight size={15} className='mr-1'/>
                                <p className="flex items-center  text-gray-500">
                                    ...
                                    <span>{selectedMascota.nombre != null?selectedMascota.nombre:'N/N'}</span>
                                </p>
                            </div>
                            <div className="flex flex-row text-sm">
                            <CornerDownRight size={15} className='mr-1'/>
                                <p className="flex items-center  text-gray-500">
                                    ...
                                    <span>{selectedMascota.edad != null?selectedMascota.edad:'N/N'}</span>
                                </p>
                            </div>
                            <div className="flex flex-row text-sm">
                            <CornerDownRight size={15} className='mr-1'/>
                                <p className="flex items-center  text-gray-500">
                                    ...
                                    <span>{selectedMascota.genero != null?selectedMascota.genero:'N/N'}</span>
                                </p>
                            </div>
                            <div className="flex flex-row text-sm">
                            <CornerDownRight size={15} className='mr-1'/>
                                <p className="flex items-center  text-gray-500">
                                    ...
                                    <span>{selectedMascota.raza != null?selectedMascota.raza:'N/N'}</span>
                                </p>
                            </div>
                            <div className="flex flex-row text-sm">
                            <CornerDownRight size={15} className='mr-1'/>
                                <p className="flex items-center  text-gray-500">
                                    ...
                                    <span>{selectedMascota.color != null?selectedMascota.color:'N/N'}</span>
                                </p>
                            </div>
                            <div className="flex flex-row text-sm">
                            <CornerDownRight size={15} className='mr-1'/>
                                <p className="flex items-center  text-gray-500">
                                    ...
                                    <span>{selectedMascota.tipo != null?selectedMascota.tipo:'N/N'}</span>
                                </p>
                            </div>
                            <div className="flex flex-row text-sm">
                            <CornerDownRight size={15} className='mr-1'/>
                                <p className="flex items-center  text-gray-500">
                                    ...
                                    <span>{selectedMascota.refugio != null?selectedMascota.refugio:'N/N'}</span>
                                </p>
                            </div>
                            <div className="flex flex-row text-sm">
                            <CornerDownRight size={15} className='mr-1'/>
                                <p className="flex items-center  text-gray-500">
                                    ...
                                    <span className={`${selectedMascota.estado_vida ==true?'text-green-600':'text-red-600'} `}>{selectedMascota.estado_vida ==true?'VIVO':'MUERTO'}</span>
                                </p>
                            </div>
                                ...
                                <div className="-mx-3 md:flex mb-2">
                                    <div className="md:w-1/2 px-3 mb-6 md:mb-0">
                                        ...
                                        <select
                                            ...
                                            value={data.estado_vida}
                                            onChange={(e) => setData('estado_vida', e.target.value)}
                                            required
                                        >
                                            <option key="4" value={4}>GENERAL</option>
                                            <option key="1" value={1}>VIVO</option>
                                            <option key="0" value={0}>MUERTO</option>
                                        </select>
                                    ...
                                    <div className="md:w-1/2 px-3 mb-6 md:mb-0">
                                        ...
                                        <select
                                            ...
                                            className="appearance-none block w-full bg-grey-lighter text-grey-darker border border-red rounded py-3 px-4 mb-3"
                                            value={data.adoptado}
                                            onChange={(e) => setData('adoptado', e.target.value)}
                                            required
                                        >
                                            <option key="4" value={4}>GENERAL</option>
                                            <option key="1" value={1}>ADOPTADO</option>
                                            <option key="0" value={0}>NO ADOPTADO</option>
                                        </select>
                                    ...
                                    <div className="md:w-1/2 px-3 mb-6 md:mb-0">
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => setData('tipo_animal', e.target.value)}
                                            required
                                        >
