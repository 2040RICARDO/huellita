...
export default function IndexCampana ({auth,campanas}) {
    ...
    const handleToggleMenu = (campanaId) => {
        setShowMenus((prevMenus) => ({
            ...prevMenus,
            [campanaId]: !prevMenus[campanaId],
        }));
    };
    ...
    const HandleDelete = (e,id) => {
        e.preventDefault();
        Swal.fire({
            title: '¿Estás seguro de que quieres eliminar la campaña?',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                router.delete(route('campana.destroy',id));
            }
        });
    };
    ...
    const searchCampanas = () => {
        return campanas.filter((campana) => {
            return (
                campana.nombre.toLowerCase().includes(searchQuery.toLowerCase())  ||
                estado.tipo.toLowerCase().includes(searchQuery.toLowerCase()) 
            );
        });
    };

    const filteredCampana = searchCampanas();
     ////////////////busqueda

     ////////////paginacion
    const [pageNumber, setPageNumber] = useState(0);
    const campanasPerPage = 30; 
    const pagesVisited = pageNumber * campanasPerPage;

    const changePage = ({ selected }) => {
        setPageNumber(selected);
    };
    const pageCount = Math.ceil(campanas.length / campanasPerPage);
    ///////////paginacion
    ...
    const handleReporte = () => {
        ...
        window.open( `/campana_r/reporte/${estado}`, '_blank');
    };
    /////REPORTE
            ...
            <div className="sm:flex sm:items-center sm:justify-between">
                <div>
                    <div className="flex items-center gap-x-3">
                        ...
                        <span className="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full  ">{campanas.length} Mascotas</span>
                    </div>

                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-300">Se tiene registrado en Total {campanas.length} Campañas</p>
                </div>
                ...
                <div className="flex items-center mt-4 gap-x-3">
                    ...
                    <Link href={route('campana.create')}  className="flex items-center justify-center w-1/2 px-5 py-2 text-sm tracking-wide text-white transition-colors duration-200 bg-blue-500 rounded-lg shrink-0 sm:w-auto gap-x-2 hover:bg-blue-600 dark:hover:bg-blue-500 dark:bg-blue-600">
                        <PlusCircle size={15}/>
                        ...
                        <div className="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg">
                            <table className="min-w-full divide-y divide-gray-200 ">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700">
                                  {
                                    filteredCampana.slice(pagesVisited, pagesVisited + campanasPerPage).map((campana,index)=>(
                                        <tr>
                                            ...
                                            <td className="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                                <div>
                                                    <h2 className="font-medium text-gray-800 te ">{campana.nombre}</h2>
                                                    <p className="text-xs font-normal text-gray-600 dark:text-gray-400"  >{campana.direccion}</p>
                                                </div>
                                            </td>
                                            
                                            <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                <h4 className="text-gray-700 ">{campana.celular}</h4> 
                                            </td>
                                            <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                <div className={`inline-flex items-center px-3 py-1 rounded-full gap-x-2 ${campana.estado == true?'text-emerald-500 bg-blue-100/60':'text-black-500 bg-pink-100/60'}  `}>
                                                           
                                                    <h2 className="text-sm font-normal">{campana.estado == true?'VIGENTE':'VENCIDO'}</h2>
                                                </div>
                                     
                                            </td>
                                            <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                <h4 className="text-gray-700 ">{campana.refugio}</h4> 
                                            </td>

                                            <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                <div>
                                                    <button
                                                        onClick={() => handleToggleMenu(campana.id)}
                    ...
                    <div className="relative flex w-full max-w-[48rem] flex-row rounded-xl bg-white bg-clip-border text-gray-700 shadow-md">
                        ...
                        <div className="flex flex-row text-sm">
                                ...
                                <p className="flex items-center  text-gray-500 ml-3 text-center">
                                    <span >{selectedCampana.descripcion != null?selectedCampana.descripcion:''}</span>
                                </p>
                            </div>
                        </div>
                        <div className="flex flex-col space-y-4  p-6 text-gray-600">
                            ...
                            <CornerDownRight size={15} className='mr-1'/>
                                <p className="flex items-center  text-gray-500">
                                    ...
                                    <span>{selectedCampana.nombre != null?selectedCampana.nombre:'N/N'}</span>
                                </p>
                            </div>
                            <div className="flex flex-row text-sm">
                            <CornerDownRight size={15} className='mr-1'/>
                                <p className="flex items-center  text-gray-500">
                                    ...
                                    <span>{selectedCampana.celular != null?selectedCampana.celular:'N/N'}</span>
                                </p>
                            </div>
                            <div className="flex flex-row text-sm">
                            <CornerDownRight size={15} className='mr-1'/>
                                <p className="flex items-center  text-gray-500">
                                    ...
                                    <span>{selectedCampana.direccion != null?selectedCampana.direccion:'N/N'}</span>
                                </p>
                            </div>
                            <div className="flex flex-row text-sm">
                            <CornerDownRight size={15} className='mr-1'/>
                                <p className="flex items-center  text-gray-500">
                                    <span className="font-semibold mr-2 text-xs uppercase">ubicacion:</span>
                                    <span>{selectedCampana.ubicacion != null?selectedCampana.ubicacion:'N/N'}</span>
                                </p>
                            </div>
                            ...
                            <CornerDownRight size={15} className='mr-1'/>
                                <p className="flex items-center  text-gray-500">
                                    <span className="font-semibold mr-2 text-xs uppercase">FECHA:</span>
                                    <span>{selectedCampana.fecha_inicio != null && selectedCampana.fecha_final != null ? `${selectedCampana.fecha_inicio} A ${selectedCampana.fecha_final}`:'N/N'}</span>
                                </p>
                            </div>
                            ...
                            <CornerDownRight size={15} className='mr-1'/>
                                <p className="flex items-center  text-gray-500">
                                    ...
                                    <span>{selectedCampana.refugio != null?selectedCampana.refugio:'N/N'}</span>
                                </p>
                            </div>
                            <div className="flex flex-row text-sm">
                            <CornerDownRight size={15} className='mr-1'/>
                                <p className="flex items-center  text-gray-500">
                                    <span className="font-semibold mr-2 text-xs uppercase">Estado campaÑa:</span>
                                    <span className={`${selectedCampana.estado ==true?'text-green-600':'text-red-600'} `}>{selectedCampana.estado ==true?'VIGENTE':'VENCIDO'}</span>
                                </p>
                            </div>
