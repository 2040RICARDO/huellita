...
export default function IndexRefugio ({auth,refugios}) {
    ...
    const handleToggleMenu = (refugioId) => {
        setShowMenus((prevMenus) => ({
            ...prevMenus,
            [refugioId]: !prevMenus[refugioId],
        }));
    };
    ...
    const HandleDelete = (e,id) => {
        e.preventDefault();
        Swal.fire({
            title: '¿Estás seguro de que quieres eliminar el refugio?',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                router.delete(route('refugio.destroy',id));
            }
        });
    };
    ...
    const searchRefugios = () => {
        return refugios.filter((refugio) => {
            return (
                refugio.nombre.toLowerCase().includes(searchQuery.toLowerCase()) ||
                refugio.direccion.toLowerCase().includes(searchQuery.toLowerCase()) ||
                refugio.ubicacion.toLowerCase().includes(searchQuery.toLowerCase()) 
            );
        });
    };

    const filteredRefugios = searchRefugios();
     ////////////////busqueda

     ////////////paginacion
    const [pageNumber, setPageNumber] = useState(0);
    const refugiosPerPage = 30; 
    const pagesVisited = pageNumber * refugiosPerPage;

    const changePage = ({ selected }) => {
        setPageNumber(selected);
    };
    const pageCount = Math.ceil(refugios.length / refugiosPerPage);
    ///////////paginacion
    ...
    const handleReporte = () => {
        ...
        window.open( `/refugio_r/reporte/${estado}`, '_blank');
    };
    /////REPORTE
  ...
  return (
    <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Refugio</h2>}
        >
            ...
            <div className="sm:flex sm:items-center sm:justify-between">
                <div>
                    <div className="flex items-center gap-x-3">
                        ...
                        <span className="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full  ">{refugios.length} Refugios</span>
                    </div>

                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-300">Se tiene registrado en Total {refugios.length} Refugios</p>
                </div>
                ...
                <div className="flex items-center mt-4 gap-x-3">
                    ...
                    <Link href={route('refugio.create')}  className="flex items-center justify-center w-1/2 px-5 py-2 text-sm tracking-wide text-white transition-colors duration-200 bg-blue-500 rounded-lg shrink-0 sm:w-auto gap-x-2 hover:bg-blue-600 dark:hover:bg-blue-500 dark:bg-blue-600">
                        <PlusCircle size={15}/>
                        ...
                        <div className="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg">
                            <table className="min-w-full divide-y divide-gray-200 ">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700">
                                  {
                                    filteredRefugios.slice(pagesVisited, pagesVisited + refugiosPerPage).map((refugio,index)=>(
                                        <tr>
                                            ...
                                            <td className="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                                <div>
                                                    <h2 className="font-medium text-gray-800 te ">{refugio.nombre}</h2>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-12 py-4 text-sm font-medium whitespace-nowrap">
                                                <div className={`inline px-3 py-1 text-sm font-normal rounded-full ${refugio.estado == 1?'text-emerald-500 gap-x-2 bg-emerald-100/60':'text-white-500 gap-x-2 bg-red-700/60'}  `}>
                                                    {refugio.estado == 1?'ABIERTO':'CERRADO'}
                                                </div>
                                            </td>
                                            <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                <div>
                                                    <h4 className="text-gray-700 ">{refugio.ubicacion}</h4>  
                                                </div>
                                            </td>
                                            <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                <h4 className="text-gray-700 ">{refugio.telefono}</h4> 
                                            </td>

                                            <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                <div>
                                                    <button
                                                        onClick={() => handleToggleMenu(refugio.id)}
                                                    ...
                                                    {showMenus[refugio.id] && (
                                                        <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg" ref={dropdownRef}>
                                                            <ul className="py-1">
                                                                ...
                                                                <li className="cursor-pointer px-4 py-2 hover:bg-gray-100">
                                                                    <button
                                                                       onClick={(e) => HandleDelete(e, refugio.id)}
                                                                        data-tooltip-id="showCliente"
                                                                        data-tooltip-content="VER"
                                                                        className="hover:text-green-900 mr-2 flex items-center"
                                                                    >
